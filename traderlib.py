...
class Trader:
    ...
    def load_historical_data(self,stock,interval='1Min',limit=100):
        ...
        timedeltaItv = ceil(int(interval.strip('Min')) * 1.5) # 150% of the interval, just in case
        ...
        while True:
            ...
            try: # check if the data is updated
                ...
                lastEntry = stock.df.last('5Min').index[0] # entry (sailing) of the last 5min
                lastEntry = lastEntry.tz_convert('utc')
                nowTimeDelta = datetime.now(timezone.utc) # now - 5min
